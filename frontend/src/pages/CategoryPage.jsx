...
const AnimatedCounter = ({ value, label }) => (
  <motion.div initial={{ scale: 0 }} animate={{ scale: 1 }} className="flex flex-col items-center">
    <span className="text-xl sm:text-2xl font-bold text-[#f4126d]">{value}</span>
...
const GlassBadge = ({ children, color = "pink" }) => {
  const colors = {
    pink: "bg-[#C2185B]/10 text-[#C2185B] border-[#C2185B]/20",
...
const CategoryPage = () => {
  ...
  if (!slug) {  // null, undefined, empty string sab check hoga
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-[#FFF5F7] px-4">
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="text-center">
          <Package size={48} className="mx-auto text-[#C2185B] mb-4 opacity-50 sm:size-16" />
          <h2 className="text-xl sm:text-2xl font-bold text-[#880E4F] mb-2">Category Not Found</h2>
          <p className="text-sm sm:text-base text-gray-500 mb-6">The category you're looking for doesn't exist.</p>
          <Link to="/" className="inline-flex items-center gap-2 px-5 sm:px-6 py-2.5 sm:py-3 bg-[#C2185B] text-white rounded-full font-semibold hover:bg-[#880E4F] transition-all text-sm sm:text-base">
  ...
  return (
    <div className="min-h-screen bg-[#FFF5F7] relative overflow-hidden">
      <div className="fixed top-20 right-0 w-64 h-64 sm:w-96 sm:h-96 bg-[#C2185B]/5 rounded-full blur-3xl pointer-events-none" />
      <div className="fixed bottom-0 left-0 w-64 h-64 sm:w-96 sm:h-96 bg-[#F8BBD9]/20 rounded-full blur-3xl pointer-events-none" />
      ...
      <div className="relative min-h-[50vh] sm:min-h-[55vh] md:min-h-[60vh] overflow-hidden flex items-center">
        <motion.div initial={{ scale: 1.2 }} animate={{ scale: 1 }} transition={{ duration: 1.5 }} className="absolute inset-0">
          <img src={currentCategory.image || "/images/banner.jpg"} alt={currentCategory.name} className="w-full h-full object-cover" />
        </motion.div>
        
        <div className="absolute inset-0 bg-[#880E4F]/80 mix-blend-multiply" />
      ...
      {currentCategory.subcategories?.length > 0 && (
        <div className="container mx-auto px-3 sm:px-4 mt-6 sm:mt-8 md:mt-12 mb-8 sm:mb-12">
          <div className="bg-white rounded-2xl shadow-xl shadow-[#C2185B]/10 p-3 sm:p-4 md:p-6 border border-[#F8BBD9]/30">
            <h3 className="text-xs sm:text-sm font-bold text-[#880E4F] uppercase tracking-wider mb-3 sm:mb-4 flex items-center gap-2">
            ...
            <div className="flex gap-2 sm:gap-3 md:gap-4 overflow-x-auto pb-3 snap-x snap-mandatory scrollbar-hide">
              <motion.button
                ...
                className={`flex-shrink-0 snap-start group relative overflow-hidden rounded-xl p-2.5 sm:p-3 md:p-4 min-w-[100px] sm:min-w-[120px] md:min-w-[130px] transition-all duration-300 ${
                  !selectedSubcat
                    ? "bg-[#C2185B] text-white shadow-lg shadow-[#C2185B]/30"
                    : "bg-[#FFF5F7] text-[#880E4F] hover:bg-[#C2185B] hover:text-white"
                }`}
              >
                <div className="relative z-10 flex flex-col items-center gap-1.5 sm:gap-2">
                  <div className={`w-8 h-8 sm:w-10 sm:h-10 md:w-12 md:h-12 rounded-full flex items-center justify-center text-sm sm:text-base md:text-lg font-bold ${
                    !selectedSubcat ? "bg-white/20" : "bg-[#C2185B]/10 group-hover:bg-white/20"
                  }`}>
                  ...
                  <span className={`text-[9px] sm:text-[10px] md:text-xs px-1.5 sm:px-2 py-0.5 rounded-full font-medium ${
                    !selectedSubcat ? "bg-white/20" : "bg-[#C2185B]/10 text-[#C2185B] group-hover:bg-white/20 group-hover:text-white"
                  }`}>
                  ...
                  >
                    <div className="relative z-10 flex flex-col items-center gap-1.5 sm:gap-2">
                      <div className={`w-8 h-8 sm:w-10 sm:h-10 md:w-12 md:h-12 rounded-full flex items-center justify-center text-sm sm:text-base md:text-lg font-bold overflow-hidden ${
                        isActive ? "bg-white/20" : "bg-[#C2185B]/10 group-hover:bg-white/20"
                      }`}>
                        {imageUrl ? (
                          <img 
                            src={imageUrl}
                      ...
                      <span className={`text-[9px] sm:text-[10px] md:text-xs px-1.5 sm:px-2 py-0.5 rounded-full font-medium ${
                        isActive 
                          ? "bg-white/20" 
                          : itemCount > 0 
                            ? "bg-[#C2185B]/10 text-[#C2185B] group-hover:bg-white/20 group-hover:text-white" 
        ...
        <div className="flex flex-col lg:flex-row gap-6 lg:gap-8">
          ...
          <AnimatePresence>
            {(showFilters || isDesktop) && (
              ...
              >
                <motion.div 
                  className={`bg-white rounded-2xl shadow-xl shadow-[#C2185B]/5 border border-[#F8BBD9]/30 p-4 sm:p-6 h-fit ${
                    showFilters && !isDesktop ? "fixed left-0 top-0 h-full w-[280px] sm:w-80 overflow-y-auto" : "w-full lg:w-72"
                  }`}
                >
                  <div className="lg:hidden flex items-center justify-between mb-6">
                    <h3 className="text-lg font-bold text-[#880E4F]">Filters</h3>
                    <button onClick={() => setShowFilters(false)} className="w-8 h-8 bg-[#FFF5F7] rounded-full flex items-center justify-center text-[#C2185B]">
                      <X size={18} />
                    </button>
                  </div>

                  <div className="mb-6">
                    <label className="text-xs sm:text-sm font-bold text-[#880E4F] mb-2 block">
                      Search in {selectedSubcatName || currentCategory.name}
                    </label>
                    <div className="relative">
                      <Search size={16} className="absolute left-3 top-1/2 -translate-y-1/2 text-[#C2185B]" />
                      <input
                        type="text"
                        value={searchQuery}
                        onChange={(e) => setSearchQuery(e.target.value)}
                        placeholder="Search products..."
                        className="w-full pl-9 sm:pl-10 pr-4 py-2.5 sm:py-3 bg-[#FFF5F7] border border-[#F8BBD9]/50 rounded-xl text-sm focus:outline-none focus:border-[#C2185B] focus:ring-2 focus:ring-[#C2185B]/20 transition-all"
                      />
                    </div>
                  </div>

                  <div className="mb-6">
                    <label className="text-xs sm:text-sm font-bold text-[#880E4F] mb-3 block">Price Range</label>
                    <div className="px-2">
                      <input
                        type="range"
                        min="0"
                      ...
                      />
                      <div className="flex justify-between mt-2 text-xs sm:text-sm font-semibold text-[#880E4F]">
                  ...
                  {(selectedSubcat || searchQuery || priceRange[1] < 50000) && (
                    <div className="mb-6">
                      <div className="flex items-center justify-between mb-2">
                        <span className="text-xs sm:text-sm font-bold text-[#880E4F]">Active Filters</span>
                        <button 
                          onClick={() => { 
                            setSelectedSubcat(null); 
                            setSearchQuery(""); 
                          ...
                          className="text-[10px] sm:text-xs text-[#C2185B] hover:underline"
                        >
                          Clear All
                        </button>
                      </div>
                      <div className="flex flex-wrap gap-1.5 sm:gap-2">
                        {selectedSubcat && (
                          <span className="inline-flex items-center gap-1 px-2 sm:px-3 py-1 bg-[#C2185B]/10 text-[#C2185B] rounded-full text-[10px] sm:text-xs font-medium">
                            {selectedSubcatName} 
                  ...
                  <div className="mb-6">
                    <label className="text-xs sm:text-sm font-bold text-[#880E4F] mb-3 block">Subcategories</label>
                    <div className="space-y-2">
                      ...
                      >
                        <div className={`w-4 h-4 sm:w-5 sm:h-5 rounded-full border-2 flex items-center justify-center transition-all ${
                          !selectedSubcat ? "bg-[#C2185B] border-[#C2185B]" : "border-[#F8BBD9] group-hover:border-[#C2185B]"
                        }`}>
                          {!selectedSubcat && <div className="w-2 h-2 bg-white rounded-full" />}
                        </div>
                        <span className={`text-xs sm:text-sm ${!selectedSubcat ? "text-[#C2185B] font-semibold" : "text-gray-600"}`}>
                          All Products
                        </span>
                      ...
                      {currentCategory.subcategories?.map(sub => (
                        ...
                        >
                          <div className={`w-4 h-4 sm:w-5 sm:h-5 rounded-full border-2 flex items-center justify-center transition-all ${
                            selectedSubcat === sub.slug ? "bg-[#C2185B] border-[#C2185B]" : "border-[#F8BBD9] group-hover:border-[#C2185B]"
                          }`}>
                            {selectedSubcat === sub.slug && <div className="w-2 h-2 bg-white rounded-full" />}
                          </div>
                          <span className={`text-xs sm:text-sm ${selectedSubcat === sub.slug ? "text-[#C2185B] font-semibold" : "text-gray-600"}`}>
                            {sub.name}
                          </span>
                  ...
                  <div className="bg-[#FFF5F7] rounded-xl p-3 sm:p-4 border border-[#F8BBD9]/30">
                    <div className="flex items-center gap-2 sm:gap-3 mb-2">
                      <div className="w-8 h-8 sm:w-10 sm:h-10 bg-[#C2185B] rounded-lg flex items-center justify-center text-white shrink-0">
                        <Sparkles size={16} className="sm:size-5" />
                      </div>
                      <div>
                        <p className="text-xs sm:text-sm font-bold text-[#880E4F]">Special Offer</p>
                        <p className="text-[10px] sm:text-xs text-gray-500">Get 20% off on first order</p>
                      </div>
                    </div>
                    <button className="w-full mt-2 py-2 bg-[#C2185B] text-white rounded-lg text-xs sm:text-sm font-semibold hover:bg-[#880E4F] transition-colors">
                      Shop Now
                    </button>
                  </div>
                </motion.div>
          ...
          <div className="flex-1 min-w-0">
            <div className="bg-white rounded-2xl shadow-sm border border-[#F8BBD9]/30 p-3 sm:p-4 mb-4 sm:mb-6 flex flex-wrap items-center justify-between gap-3">
              <div className="flex items-center gap-3">
                <button onClick={() => setShowFilters(true)} className="lg:hidden flex items-center gap-1.5 sm:gap-2 px-3 sm:px-4 py-1.5 sm:py-2 bg-[#FFF5F7] text-[#C2185B] rounded-lg font-medium text-xs sm:text-sm">
                  <SlidersHorizontal size={16} /> Filters
                </button>
                <span className="text-xs sm:text-sm text-gray-500">
                  Showing <span className="font-bold text-[#880E4F]">{filteredProducts.length}</span> of {products?.length || 0} products
                  {selectedSubcatName && <span className="text-[#C2185B]"> in {selectedSubcatName}</span>}
                </span>
              </div>
              ...
              <div className="flex items-center gap-2 sm:gap-3">
                <div className="relative">
                  <button 
                    ...
                    className="flex items-center gap-1.5 sm:gap-2 px-3 sm:px-4 py-1.5 sm:py-2 bg-[#FFF5F7] text-[#880E4F] rounded-lg text-xs sm:text-sm font-medium hover:bg-[#F8BBD9]/20 transition-colors"
                  >
                    <ArrowUpDown size={14} />
                  ...
                  <AnimatePresence>
                    {showSortDropdown && (
                      <motion.div
                        ...
                        exit={{ opacity: 0, y: 10 }}
                        className="absolute right-0 top-full mt-2 w-52 sm:w-56 bg-white rounded-xl shadow-xl shadow-[#C2185B]/10 border border-[#F8BBD9]/30 overflow-hidden z-20"
                      >
                        {sortOptions.map(option => (
                          <button
                            ...
                            className={`w-full flex items-center gap-2 sm:gap-3 px-3 sm:px-4 py-2.5 sm:py-3 text-xs sm:text-sm transition-colors ${
                              sortBy === option.value ? "bg-[#FFF5F7] text-[#C2185B] font-semibold" : "text-gray-600 hover:bg-[#FFF5F7]"
                            }`}
                          >
                            <option.icon size={14} />
                            {option.label}
                ...
                <div className="hidden sm:flex bg-[#FFF5F7] rounded-lg p-1">
                  <button 
                    ...
                    className={`p-1.5 rounded-md transition-all ${viewMode === "grid" ? "bg-white text-[#C2185B] shadow-sm" : "text-gray-400"}`}
                  >
                    <Grid3X3 size={16} />
                  </button>
                  <button 
                    onClick={() => setViewMode("list")} 
                    className={`p-1.5 rounded-md transition-all ${viewMode === "list" ? "bg-white text-[#C2185B] shadow-sm" : "text-gray-400"}`}
                  >
                    <LayoutList size={16} />
                  </button>
                </div>
            ...
            {productsLoading ? (
              ...
              <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="text-center py-16 sm:py-20">
                <Package size={48} className="mx-auto text-[#C2185B] mb-4 opacity-30 sm:size-16" />
                <h3 className="text-lg sm:text-xl font-bold text-[#880E4F] mb-2">No Products Found</h3>
                ...
                <div className="flex gap-2 justify-center flex-wrap">
                  {selectedSubcat && (
                    <button 
                      onClick={() => { setSelectedSubcat(null); navigate(`/category/${categorySlug}`); }} 
                      className="px-5 sm:px-6 py-2 sm:py-2.5 bg-[#C2185B] text-white rounded-full font-medium hover:bg-[#880E4F] transition-colors text-sm"
                    >
                      Show All Products
                    </button>
                  )}
                  <button 
                    ...
                    className="px-5 sm:px-6 py-2 sm:py-2.5 border-2 border-[#C2185B] text-[#C2185B] rounded-full font-medium hover:bg-[#FFF5F7] transition-colors text-sm"
                  >
                    Clear Filters
                  </button>
                </div>
              ...
              <motion.div layout className={`grid gap-4 sm:gap-6 ${
                ...
                <AnimatePresence>
                  {filteredProducts.map((product, index) => (
                    ...
                    >
                      <div className={`relative overflow-hidden bg-[#FFF5F7] ${
                        ...
                        <div className="absolute inset-0 bg-[#880E4F]/0 group-hover:bg-[#880E4F]/20 transition-all duration-300 flex items-center justify-center gap-2 sm:gap-3 opacity-0 group-hover:opacity-100 pointer-events-none">
                          <motion.button 
                            whileHover={{ scale: 1.1 }} 
                            whileTap={{ scale: 0.9 }} 
                            onClick={(e) => { e.stopPropagation(); setQuickView(product); }}
                            className="w-10 h-10 sm:w-12 sm:h-12 bg-white rounded-full flex items-center justify-center text-[#C2185B] shadow-lg pointer-events-auto"
                          >
                      ...
                      <Link to={`/product/${product._id}`} className="p-3 sm:p-5 flex-1 flex flex-col min-w-0 block">
                        <div className="flex items-start justify-between gap-2 mb-1.5 sm:mb-2">
                          <div className="min-w-0">
                            <p className="text-[10px] sm:text-xs text-[#C2185B] font-semibold uppercase tracking-wider mb-0.5 sm:mb-1">
                              {product.subcategory || product.subCategory || product.category?.sub || currentCategory.name}
                            </p>
                            <h3 className="font-bold text-[#2D2D2D] group-hover:text-[#C2185B] transition-colors line-clamp-2 text-sm sm:text-base">
                              {product.name}
                            </h3>
                          </div>
                        </div>
                        ...
                        <div className="flex items-end justify-between mt-auto pt-2 sm:pt-3 border-t border-[#F8BBD9]/20">
                          <div>
                            <p className="text-lg sm:text-2xl font-bold text-[#C2185B]">
                          ...
                          <motion.button 
                            ...
                            className="w-10 h-10 sm:w-12 sm:h-12 bg-[#C2185B] hover:bg-[#880E4F] text-white rounded-xl flex items-center justify-center shadow-lg shadow-[#C2185B]/30 transition-colors shrink-0"
                          >
          ...
          >
            ...
            >
              <div className="flex flex-col md:grid md:grid-cols-2">
                <div className="relative bg-[#FFF5F7] p-4 sm:p-8 flex items-center justify-center">
                  <img src={quickView.images?.[0] || quickView.image} alt={quickView.name} className="max-h-48 sm:max-h-80 object-contain" />
                  <button onClick={() => setQuickView(null)} className="absolute top-3 right-3 sm:top-4 sm:right-4 w-8 h-8 sm:w-10 sm:h-10 bg-white rounded-full flex items-center justify-center text-[#C2185B] shadow-md">
                    <X size={16} className="sm:size-5" />
                  </button>
                </div>

                <div className="p-4 sm:p-8">
                  <p className="text-xs sm:text-sm text-[#C2185B] font-bold uppercase tracking-wider mb-2">
                    {quickView.subcategory || quickView.subCategory || quickView.category?.sub || currentCategory.name}
                  </p>
                  <h2 className="text-xl sm:text-3xl font-bold text-[#2D2D2D] mb-2 sm:mb-4">{quickView.name}</h2>
                  <p className="text-sm text-gray-600 mb-4 sm:mb-6 leading-relaxed">{quickView.description}</p>
                  
                  <div className="flex items-center gap-3 sm:gap-4 mb-4 sm:mb-6">
                    <span className="text-2xl sm:text-3xl font-bold text-[#C2185B]">Rs. {quickView.discountedPrice || quickView.price}</span>
                  ...
                  <div className="flex gap-2 sm:gap-3">
                    <button 
                      ...
                      className="flex-1 py-3 sm:py-4 bg-[#C2185B] text-white rounded-xl font-bold hover:bg-[#880E4F] transition-colors flex items-center justify-center gap-2 text-sm sm:text-base"
                    >
                      <ShoppingCart size={18} className="sm:size-5" /> Add to Cart
                    </button>
                    <button 
                      onClick={() => navigate(`/product/${quickView._id}`)} 
                      className="px-4 sm:px-6 py-3 sm:py-4 border-2 border-[#C2185B] text-[#C2185B] rounded-xl font-bold hover:bg-[#FFF5F7] transition-colors text-sm sm:text-base"
                    >
                      Details
                    </button>
                  </div>
      ...
      <motion.button
        ...
        animate={{ scale: 1 }}
        whileHover={{ scale: 1.1 }}
        className="fixed bottom-4 right-4 sm:bottom-6 sm:right-6 md:bottom-8 md:right-8 w-12 h-12 sm:w-14 sm:h-14 bg-[#C2185B] text-white rounded-full shadow-2xl shadow-[#C2185B]/40 flex items-center justify-center z-40"
      >
